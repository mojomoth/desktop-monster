...
| F21 | Canvas scene & HUD boot render | Renderer draws the full scene each rAF frame: field strip, hero left and monster right at `SPRITE_SCALE = 2` (Assumption 17), boxed HP bar above monster, `LV n`+XP bar floating above the hero's head, top-right kill and coin counters, pooled floating damage numbers; first painted frame reported via IPC (drives F18) | `npm run smoke > /tmp/desmon-smoke.log 2>&1 && grep -q SMOKE_OK /tmp/desmon-smoke.log` → exit 0 (proves the scene painted without renderer exception; layout/quality: Manual M2, M3) |
