# Lane T53 — Builder (iteration 48)

You are a fresh agent in an isolated git worktree. Everything you need is in
this prompt and on disk. Your job: complete EXACTLY ONE task — T53
"SPEC criteria sweep (F01–F58, Server / API, Deployment — literal)" — with gates green, committed in this worktree, then report with
one JSON object. Then stop.

- Working directory: /Users/jeongyounglee/work/repo/desktop-monster/.worktrees/T53 (branch `lane/T53`; `node_modules` is a
...
- Work ONLY inside /Users/jeongyounglee/work/repo/desktop-monster/.worktrees/T53. The main checkout (two directories up) is off
...
5. Any file in `.agentdoc/2026-09-03T00-02-53/sessions/` whose name or text mentions
   T53 (attempts & dead ends of earlier tries), if present.
...
The heading of T53 on disk must be `[~]`. If it is not, or the block above
...
- If the task is too big for one iteration: implement nothing, leave the tree
  clean, report `result: "SPLIT"` with `children` (ids `T53a`,
  `T53b`…, each with title/worker/files/deps/ac; `files` complete
  including tests).
...
`git add -A && git commit -m "<type>(T53): <imperative subject>"`
...
Write `.agentdoc/2026-09-03T00-02-53/sessions/iter-48.md` following
...
`- worker: claude`, `- lane: .worktrees/T53 (branch lane/T53)`) and
...
{"task":"T53","result":"DONE|SPLIT|BLOCKED|NOTHING_TO_DO|MISMATCH","gates":"pass|fail","commit":"<sha or none>","note":"<what you did + dead ends, <=600 chars>","children":[]}
