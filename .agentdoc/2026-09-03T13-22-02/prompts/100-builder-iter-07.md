# Lane T67 — Builder (iteration 07)

You are a fresh agent in an isolated git worktree. Everything you need is in
this prompt and on disk. Your job: complete EXACTLY ONE task — T67
"Net client + session v3: match, pvp(matchId, party), thefts, reclaim, toSnapshot party, identity notifiedTheftIds" — with gates green, committed in this worktree, then report with
one JSON object. Then stop.

- Working directory: /Users/jeongyounglee/work/repo/desktop-monster/.worktrees/T67 (branch `lane/T67`; `node_modules` is a
...
- Work ONLY inside /Users/jeongyounglee/work/repo/desktop-monster/.worktrees/T67. The main checkout (two directories up) is off
...
5. Any file in `.agentdoc/2026-09-03T13-22-02/sessions/` whose name or text mentions
   T67 (attempts & dead ends of earlier tries), if present.
...
The heading of T67 on disk must be `[~]`. If it is not, or the block above
...
- If the task is too big for one iteration: implement nothing, leave the tree
  clean, report `result: "SPLIT"` with `children` (ids `T67a`,
  `T67b`…, each with title/worker/files/deps/ac; `files` complete
  including tests).
...
`git add -A && git commit -m "<type>(T67): <imperative subject>"`
...
Write `.agentdoc/2026-09-03T13-22-02/sessions/iter-07.md` following
...
`- worker: claude`, `- lane: .worktrees/T67 (branch lane/T67)`) and
...
{"task":"T67","result":"DONE|SPLIT|BLOCKED|NOTHING_TO_DO|MISMATCH","gates":"pass|fail","commit":"<sha or none>","note":"<what you did + dead ends, <=600 chars>","children":[]}
