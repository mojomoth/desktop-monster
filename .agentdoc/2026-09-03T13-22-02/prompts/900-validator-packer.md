...
3. Spot-check: pick 3 `[x]` tasks at random from T54+ PLUS every task whose
   title mentions window/input/package/smoke/server/deploy/party/battle/theft/field PLUS every
...
6. Package: `npm run package`; assert the DesMon .app exists under `release/`
   (`release/mac-arm64/DesMon.app`) and `release/DesMon-0.3.0-arm64.dmg` exists.
...
7. Deploy verification (ONCE with network; reruns read the archived log):
   ...
   unreachable → you may fix it once: `git push origin v3` (the integration branch of this harness; never main), look up the
...
- Final push (the only routine push in v2 besides the deploy task):
  `git push origin v3` (the integration branch of this harness; never main) — skipped under `DESMON_SKIP_NET=1`; a rejected push
...
## Session parameters (appended by desmon-3-eval)

Active session dir: .agentdoc/2026-09-03T13-22-02
SERVER_URL=https://desmon-server-v3.onrender.com
Integration branch: v3 (DESMON_BASE_BRANCH=v3). Wherever this charter says `main`, read `v3`: run in the main checkout on branch `v3`; `git merge-base --is-ancestor <sha> HEAD` is evaluated against the `v3` HEAD; the final push is `git push origin v3`; never touch `main`.
Harness version: v3 (HV=v3)
Dev-loop outcome (.agentdoc/2026-09-03T13-22-02/sessions/dev-loop.md):
...
DESMON_SKIP_NET=unset

Your final message MUST begin with `VERDICT: CONVERGED` or `VERDICT: NOT_CONVERGED`, followed by a compact summary of every protocol step 0-12 with its result (command + exit), the handoff path, the release artifact paths, the healthz JSON, and the list of any tasks you flipped back to `[ ]`. Do not ask questions; there is no human in the loop.

## Spawn wrapper (verbatim text given to the subagent; it reads this file first)

You are the Validator/Packer subagent of the Desktop Monster harness (stage 3, harness v3). Your complete charter and the session parameters are in the file `.agentdoc/2026-09-03T13-22-02/prompts/900-validator-packer.md` in the repo root `/Users/jeongyounglee/work/repo/desktop-monster`. Read that file FIRST, in full, and follow it exactly, in order (steps 0–12), logging every command and result to `.agentdoc/2026-09-03T13-22-02/sessions/stage3-eval.md` (`worker: claude`, `lane: main`).

Overrides for this spawn:
- Skip the final routine `git push origin v3` under "Logging duties" — the orchestrator makes that single push after its own independent verification. The deploy-fix push in step 7 stays allowed only if the deploy is stale or unreachable.
- Do commit your artifacts: `git add -A && git commit -m "chore(eval): stage-3 validation and handoff [harness v3]"`.
- There is no human in the loop: never ask questions; decide, record, move on.
- Your final text is returned verbatim to the orchestrator as data (not shown to a human); it MUST begin with `VERDICT: CONVERGED` or `VERDICT: NOT_CONVERGED`, then a compact per-step summary (0–12: command + exit), the handoff path, the release artifact paths, the verbatim healthz JSON, and any tasks flipped back to `[ ]`.
