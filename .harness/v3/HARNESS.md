...
rules); `bash .harness/v3/loop/iterate.sh selftest` is its runnable check.
Tool facts: `reference/TOOLING.md`. Code-style ruleset: `reference/PONYTAIL.md`.

## 0. Integration branch (v3)

Every `git` operation of the loop targets `$BASE` = `DESMON_BASE_BRANCH` (default
`main`; harness v3 runs with `DESMON_BASE_BRANCH=v3`): lanes fork from it,
`collect` merges into it, `verdict` checks it, deploy tasks push `HEAD:v3`,
stage 3 pushes `origin v3`. `main` holds v2 (tag `v2`) until the user merges.

## 1. Loop modes

Both modes run the SAME engine, `bash .harness/v3/loop/iterate.sh loop` (§4);
...
- STANDALONE: `bash .harness/v3/loop/ralph.sh [--max-iterations N] [--lanes N] [--version vN]`
...
> Run the Desktop Monster harness v3 end to end without asking me anything:
...
Scripts under `.harness/v3/loop/`: `plan.mjs` (plan-file toolkit; node ESM,
...
`bash .harness/v3/loop/iterate.sh collect <id>` (if `$S/lanes/<id>.rc` exists)
or `git worktree remove --force .worktrees/<id>` +
`node .harness/v3/loop/plan.mjs set-status <id> ' '` + `rm -f $S/lanes/<id>.*`
...
Canonical skills live in `.harness/v3/skills/`. Install with:
`cp -R .harness/v3/skills/. .claude/skills/`. Never hand-edit `.claude/skills/`
...
session dir, then run the loop there:
    ...
    RUNNER=standalone NESTED_CLAUDE=1 LANES=2 MAX_ITER=2 POLL_SECONDS=10 bash .harness/v3/loop/iterate.sh loop
...
v1 and v2 are FROZEN (their `.agentdoc` sessions must stay re-readable against the
exact prompts that produced them); v3 freezes the moment its first `dispatch` runs.
Mechanics: `cp -R .harness/v3 .harness/v4` → edit v4 → CHANGELOG entry →
`bash .harness/v4/loop/iterate.sh selftest` → `printf v4 > .harness/CURRENT`
→ re-sync skills (§10) → commit `chore(harness): bump to v4 — <reason>`.
