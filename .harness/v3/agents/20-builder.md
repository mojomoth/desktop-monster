...
- Work ONLY inside your worktree `{{LANE_DIR}}` (a git worktree of this repo on
  ...
  `git checkout main` or `git checkout v3` (the integration branch), `git worktree`, `git merge`, `git rebase`.
...
command the Notes give (`git push origin HEAD:v3` — the integration branch of this harness) after your commit and
...
The orchestrator flipped `{{TASK}}` to `[~]` on the integration branch (`v3`) BEFORE creating your
...
orchestrator merges `lane/{{TASK}}` into the integration branch (`v3`) with `--no-ff`, re-runs gates,
and writes the plan; you never touch the integration branch.
