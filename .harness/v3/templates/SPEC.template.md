...
- Host: Render free tier, region oregon; web service `desmon-server-v3` (branch `v3`; v2 stays on `desmon-server`)
  (`npm ci --include=dev --ignore-scripts && npm run build`, start
  `npm run start:server`, health path `/healthz`), Postgres `desmon-db` (shared with v2).
...
- Health contract: `GET $SERVER_URL/healthz` → `200 {"ok":true,"sha":"<sha>"}`
  where `<sha>` is an ancestor of the integration branch HEAD (`v3`) and no commit after it touches the
