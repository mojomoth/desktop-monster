...
## 8. v3 additions (harness v3, 2026-09-03)

- Integration branch `v3` (`DESMON_BASE_BRANCH=v3` for `iterate.sh`/`ralph.sh`);
  `main` = v2 (tag `v2`).
- Second web service `desmon-server-v3`: `DESMON_SRV_NAME=desmon-server-v3 DESMON_BRANCH=v3 bash .harness/v3/loop/render-bootstrap.sh`
  (`render services create … --branch v3`, same build/start/filter settings,
  same `DATABASE_URL` = shared `desmon-db`, expires 2026-10-03). Expected
  `SERVER_URL=https://desmon-server-v3.onrender.com`. `render services update <srv> --branch <b>`
  exists in CLI 2.26.0 (not used: v2 keeps its own service).
- v2 run lessons: `.gitignore` `node_modules/` did not ignore lane SYMLINKS
  (fixed: bare `node_modules`, `graphify-out`); the background loop must run
  with `</dev/null` (npm "reading 'stdin'" error otherwise); `claude -p` can hit
  the account session limit (HTTP 429 → CRASHED ×3 → exit 3): wait for the
  reset, then re-run desmon-2-dev (HARNESS §5).
