...
#   iterate.sh collect <id>      merge a finished lane into $BASE, verify, write the plan, export logs
...
BASE="${DESMON_BASE_BRANCH:-main}"   # integration branch: lanes fork from it and merge back into it (v3 runs on the v3 branch)
...
commit_stray() {  # <context>: live lane output under .agentdoc is expected (collect commits it); anything else on $BASE is a stray change
...
dispatch() {
  ...
  for id in $(ready_ids); do
    ...
    git worktree add -q -b "lane/$id" "$lane" "$BASE" || { note "worktree add failed for $id"; plan set-status "$id" ' '; git add "$PLAN"; git commit -qm "docs(agentdoc): undispatch $id [ralph]"; rm -f "$S/lanes/$id".*; continue; }
...
collect() {
  ...
  # merge into $BASE for reportable results
  commit_stray "collect $ITER"
  case "$RESULT" in
    DONE|SPLIT|BLOCKED|NOTHING_TO_DO)
      if git show-ref --verify --quiet "refs/heads/lane/$id" && [ "$(git rev-parse "lane/$id")" != "$(git merge-base "$BASE" "lane/$id")" ]; then
        if git merge --no-ff -q -m "merge($id): $TITLE [$WORKER iter $ITER]" "lane/$id" >> "$S/sessions/iter-$ITER.merge.log" 2>&1; then
          ...
          if [ "$GATES_R" = fail ] || [ "$SMOKE" = fail ]; then
            if git revert --no-edit -m 1 HEAD >> "$S/sessions/iter-$ITER.merge.log" 2>&1; then
              ...
              RESULT=MERGE_RED; NOTE="MERGE LEFT ON $BASE: revert failed (gates=$GATES_R smoke=$SMOKE) — fix $BASE by hand; $NOTE"; note "revert of merge $SHA FAILED"
            fi
            kept="lane/$id-red-$ITER"
          fi
        else
          git merge --abort >/dev/null 2>&1; RESULT=CONFLICT; NOTE="merge conflict with $BASE; $NOTE"; kept="lane/$id-conflict-$ITER"
        fi
      else
        MERGE=empty
      fi ;;
...
verdict() {  # exit 0 converged · 2 blocked escalation · 1 cap · 4 deadlock / final gates red
  ...
  if [ -z "$open" ]; then
    ...
    note "final gates/smoke RED on $BASE"; return 4
  fi
...
loop() {
  ...
  [ "$(git rev-parse --abbrev-ref HEAD)" = "$BASE" ] || { echo "FATAL: not on $BASE (DESMON_BASE_BRANCH)" >&2; return 65; }
