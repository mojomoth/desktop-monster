---
name: desmon-1-plan
description: Stage 1 of the Desktop Monster harness (v2, brownfield). Runs the loop selftest, opens a NEW .agentdoc/{ts} session with tool versions, refreshes the graphify graph, then spawns the Spec Clarifier (AMEND mode — extends SPEC.md, F01–F27 kept) and the Planner (APPEND mode — adds T22+ tasks with Worker/Deps/Files/AC to IMPLEMENTATION_PLAN.md; T01–T21 and the Iteration Log stay immutable). Use when starting a harness run or when the spec/plan must be extended.
---

# desmon-1-plan — Spec & Plan stage (v2)
...
3. ALWAYS open a new session (one session ↔ one harness version):
   ...
   `harness_version=$HV`, `runner=in-session`, `started`, `lanes=${LANES:-3}`,
...
1. Compose the prompt: contents of `.harness/$HV/agents/00-spec-clarifier.md`
   + `Active session dir: $S` + `Mode: AMEND (SPEC.md exists; extend it, keep F01–F27)`.
...
4. Verify SPEC.md: non-empty, contains no "TBD"; every feature row has an
   `AC:`; rows F01–F27 still present; new rows (F28+) carry a `Worker` value;
...
1. Prompt = `.harness/$HV/agents/10-planner.md` + `Active session dir: $S`
   + `Mode: APPEND (T01–T21 and their Iteration Log rows are immutable; new tasks are T22+)`.
...
2. Validate IMPLEMENTATION_PLAN.md — ALL must hold:
   ...
   - every NEW task block (T22+) has `- AC:`, `- Deps:`, `- Worker: (claude|codex)`
   ...
   - every `Worker: codex` task: all `Files:` paths (ignoring SPEC.md,
