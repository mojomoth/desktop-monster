...
5. The last 2 files matching `{{SESSION_DIR}}/sessions/iter-*.md`, if any — for
...
- Append ONE row to the Iteration Log table (iter {{ITER}}, task, result,
  gates, commit sha, one-line note). Never edit existing rows or other tasks.

## 7. Write your session record

Create `{{SESSION_DIR}}/sessions/iter-{{ITER}}.md` following
